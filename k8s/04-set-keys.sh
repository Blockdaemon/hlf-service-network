...
for peer in 0 1; do
    ...
    set-configmap peer${peer}-msp \
        --from-file="ca-cert.pem"="${PEER_DIR}/msp/cacerts/ca.org1.${PEM}" \
        --from-file="tlsca-cert.pem"="${PEER_DIR}/msp/tlscacerts/tlsca.org1.${PEM}"  \
        --from-file="admin-cert.pem"="${PEER_DIR}/msp/admincerts/Admin@org1.${PEM}" \
        --from-file="sign-cert.pem"="${PEER_DIR}/msp/signcerts/peer${peer}.org1.${PEM}"
